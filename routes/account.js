...
router.post('/account/join', async (req, res) => {
  ...
  const validateResult = joiSchema.validate(req.dody);
  if (validateResult.error) {
    res.status(400).json({ error: '입력된 값이 잘못됐습니다.'});
    return;
  }
...
//비밀번호는 평문으로 쓰지말고 해싱해서 저장
//단방향 암호화
  const hashedPassword = await bcrypt.hash(password, 10); //비크립트 해시는 솔트를 생성
